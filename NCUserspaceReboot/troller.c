...
//  usprebooter
//
//  Created by Nick Chan on 29/11/2023.
//
...
//  copyfile("/sbin/launchd", "/private/preboot/launchd", NULL, COPYFILE_ALL);
    return ret;
}
int userspaceReboot(void) {
    ...
    if (xpc_get_type(connection) == XPC_TYPE_ERROR) {
        char* desc = xpc_copy_description(random);
        NSLog(CFSTR("%s"),desc);
        free(desc);
